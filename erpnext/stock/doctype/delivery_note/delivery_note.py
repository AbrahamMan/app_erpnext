...
class DeliveryNote(SellingController):
	...
	# def update_pkg_net_weight(self):
	# 	total_weight = sum(item.qty*item.net_weight for item in self.get("delivery_note_details"))
	# 	total_weight = 0
	# 	for item in self.get("delivery_note_details"):
	# 		total_weight = item.qty * float(item.net_weight) + total_weight
	# 	self.pkg_net_weight = total_weight
...
def make_sales_invoice(source_name, target_doc=None):
	...
	doc = get_mapped_doc("Delivery Note", source_name, 	{
		...
		"Delivery Note": {
			"doctype": "Sales Order",
			"field_map": {
				"freight_region_code": "freight_region_code"
			}
		},
		"Delivery Note Item": {
